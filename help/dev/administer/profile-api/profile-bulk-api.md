...
## [!DNL Bulk Profile Update API] での空の値の処理の説明

[!DNL Target] [!DNL Bulk Profile Update API] （v1 または v2）を使用する場合、システムが空のパラメーターや属性値をどのように処理するかを理解することが重要です。

### 予期される動作

既存のパラメーターまたは属性の空の値（「」、null、または欠落しているフィールド）を送信しても、プロファイルストア内のそれらの値はリセットまたは削除されません。 これは仕様によるものです。

空の値は無視されます。API は、処理中に空の値をフィルターで除外して、不要な更新や意味のない更新を回避します。

**既存データの消去なし**：パラメーターに既に値が存在する場合、空の値を送信すると値は変更されません。

**空のバッチのみがスキップされます**：バッチに空の値または null 値のみが含まれる場合、そのバッチは完全に無視され、更新は適用されません。

### 追加情報

この動作は、[!DNL Bulk Profile Update API] の v1 と v2 の両方に適用されます。

空の値を送信して属性を消去または削除しようとしても、効果はありません。

明示的な属性の削除は、今後のバージョンの API （v3）でサポートされる予定ですが、現在は利用できません。