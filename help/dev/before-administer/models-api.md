...
Models API を使用する場合、&lbrace;Target Admin API[&#x200B; を使用する場合と同様に、&lbrace;0](https://developer.adobe.com/console/home)Adobe Developer Console[&#x200B; を使用して認証を設定する必要があります。 &#x200B;](../administer/admin-api/admin-api-overview-new.md)詳しくは、[&#x200B; 認証の設定方法 &#x200B;](../before-administer/configure-authentication.md) を参照してください。
...
* `blockedFeatures` から識別された値を `internalName` に入力します。 [&#x200B; 手順 1](#step1) を参照してください。
...
この例では、ユーザーは `SES_PREVIOUS_VISIT_COUNT` と `SES_TOTAL_SESSIONS` の 2 つの機能をブロックしています。これは、手順 1[&#x200B; で説明したように、アクティビティ ID が 260480](#step1) のアクティビティの機能の完全なリストをクエリすることで、以前に識別されました。 また、前述の [&#x200B; 表 &#x200B;](#table) で説明されているように、「Experience Cloud」のプレフィックスで機能をブロックすることで実現される、AAM Segments からのすべての機能もブロックします。

![手順 3](assets/models-api-step-3.png)

機能をブロックリストに加えるした後で、[&#x200B; 手順 2](#step2) を再度実行して、更新されたブロックリストを確認することをお勧めします（GET ブロックリストに加える the）。 結果が期待どおりに表示されることを確認します（結果に、最新のPUT リクエストから追加された機能が含まれていることを確認します）。
...
いつものように、ブロックリストを修正した後で、もう一度 [&#x200B; 手順 2](#step2) を実行することをお勧めします（リストに想定どおりの機能が含まれていることを確認するGET ブロックリストに加える）。 ここに示す例では、ユーザーは自分のブロックリストが空であることを確認しています。
...
上記の例はすべて、1 つのアクティビティのコンテキストに含まれていました。 各アクティビティにブロックリストを個別に指定する代わりに、特定のクライアント（テナント）にわたるすべてのアクティビティの機能をブロックすることもできます。 グローバルブロックリストを実行するには、`/blockList/global` の代わりに `blockList/<campaignId>` 呼び出しを使用します。
...
上記のリクエストのサンプルでは、ユーザーが、[!DNL Target] アカウント内のすべてのアクティビティで「AAM_FEATURE_1」と「AAM_FEATURE_2」という 2 つの機能をブロックしています。 つまり、アクティビティに関係なく、「AAM_FEATURE_1」と「AAM_FEATURE_2」は、このアカウントの機械学習モデルには含まれません。 さらに、プレフィックスが「AAM」、「PRO」または「ENV」の機能はすべてグローバルにブロックされています。

質問：上記のコードサンプルは冗長ではありませんか？

回答：はい。 「AAM」で始まる値を持つ機能をブロックし、ソースが「AAM」のすべての機能をブロックすることは冗長です。 最終的には、AAM（Experience Cloud Segments）をソースとするすべての機能がブロックされます。 したがって、Experience Cloud Segments のすべての機能をブロックすることを目的とする場合、上記の例では、「AAM」で始まる特定の機能を個別に指定する必要はありません。

最終手順：アクティビティレベルでもグローバルレベルでも、変更後にブロックリストを検証し、期待する値が含まれていることを確認することをお勧めします。 これを行うには、`PUT` を `GET` に変更します。

以下に示す応答の例では、2 つの個別の機能に加え、「AAM」、「PRO」および「ENV」をソースとす [!DNL Target] すべての機能がブロックされていることを示しています。
