...
+ クライアント側実装 {#client-side}
   ...
   + [ユーザーエージェントとクライアントのヒント](implement/client-side/atjs/user-agent-and-client-hints.md)
...
+ サーバー側の実装 {#server-side}
   ...
   + 主な原則 {#core-principles}
      + [コア原則の概要](implement/server-side/sdk-guides/core-principles/overview.md)
      + [ユーザー ID とグループ化](implement/server-side/sdk-guides/core-principles/user-identification-and-bucketing.md)
   ...
   + オンデバイス判定 {#on-device-decisioning}
      ...
      + [機能フラグを使用して A/B テストを実行する](implement/server-side/sdk-guides/on-device-decisioning/execute-ab-tests-with-feature-flags.md)
      + [属性を使用して機能テストを実行](implement/server-side/sdk-guides/on-device-decisioning/execute-feature-tests-with-attributes.md)
      + [機能テストのロールアウトを管理](implement/server-side/sdk-guides/on-device-decisioning/manage-rollouts-for-feature-tests.md)
      + [パーソナライゼーションを配信](implement/server-side/sdk-guides/on-device-decisioning/deliver-personalization.md)
   ...
   + Node.js SDK リファレンス {#node-js}
      ...
      + [Node.js SDK を初期化します。](implement/server-side/node-js/initialize-sdk.md)
   ...
   + Java SDK リファレンス {#java}
      ...
      + [非同期リクエスト (Java)](implement/server-side/java/asynchronous-requests.md)
   ...
   + .NET SDK リファレンス {#net}
      ...
      + [.NET SDK を初期化します。](implement/server-side/net/initialize-sdk.md)
      + [オファーの取得 (.NET)](implement/server-side/net/get-offers.md)
      + [属性の取得 (.NET)](implement/server-side/net/get-attributes.md)
      + [通知の送信 (.NET)](implement/server-side/net/send-notifications.md)
      + [SDK イベント (.NET)](implement/server-side/net/sdk-events.md)
      + [非同期リクエスト (.NET)](implement/server-side/net/asynchronous-requests.md)
...
+ モバイルアプリの実装 {#mobile-apps}
   ...
   + Adobeモバイル SDK バージョン 4.x {#version-4}
      + [モバイルアプリにおける Target の仕組み](implement/mobile/how-target-works-mobile-apps.md)
      + [SDK での Target の有効化](implement/mobile/enable-target-in-sdk.md)
      + [iOS - Target の位置と成功指標の作成](implement/mobile/mobile-create-location-and-metric.md)
      + [iOS - カスタムユーザーデータの送信](implement/mobile/mobile-custom-user-data.md)
      + [プリフェッチオファーコンテンツ](implement/mobile/prefetch-offer-content.md)
...
+ API ガイド {#api}
   ...
   + 配信 API ガイド {#delivery-api}
      ...
      + [Delivery API とやり取りする SDK](/help/dev/before-implement/delivery-api-overview/sdks.md)
   ...
   + Recommendations API {#recommendations-api}
      ...
      + [Recommendationsでの Delivery API の使用](before-administer/recs-api/fetch-recs-server-side-delivery-api.md)
      + [Recommendations API](/help/dev/administer/recommendations-api/recommendations-api.md)
   + モデル API {#models-api}
      + [モデル API(ブロックリストに加える) の概要](before-administer/models-api.md)
