...
>[!NOTE]
>
>新しい訪問者の場合、Visitor APIが存在する場合、[!DNL Target]は複数回ワイヤーを確認して、[!DNL Target] リクエストにExperience Cloud Visitor ID データが含まれていることを確認する必要があります。 再訪問者の場合、[!DNL Target] は、パーソナライズされたコンテンツを取得するためにのみ [!DNL Target] とやり取りします。
...
FP-11577 （またはそれ以降）を使用する[!DNL Adobe Experience Manager] 6.2では、[!UICONTROL Adobe Target Cloud Services]統合でat.js実装をサポートするようになりました。
...
はい、at.js 2.xを使用する場合は、SPAにVECを使用できます。 詳しくは、[単一ページ （SPA） Visual Experience Composer](https://experienceleague.adobe.com/docs/target/using/experiences/spa-visual-experience-composer.html)を参照してください。
...
* ランダムな文字で構成されています
* セッション IDは30分間有効です
* 応答トークンを無効にできます（[応答トークン &#x200B;](https://experienceleague.adobe.com/docs/target/using/administer/response-tokens.html)）
