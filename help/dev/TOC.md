...
+ はじめに {#implementation}
   ...
   + プライバシーとセキュリティ {#privacy}
      ...
      + [サードパーティクッキーの廃止がTarget （at.js）に与える影響](/help/dev/before-implement/privacy/third-party-cookie-deprecation.md)
   ...
   + データを Target に送信する方法 {#methods}
      ...
      + [プロファイルの一括更新API](before-implement/methods-to-get-data-into-target/bulk-profile-update-api.md)
      + [単一プロファイル更新API](before-implement/methods-to-get-data-into-target/single-profile-update-api.md)
...
+ クライアントサイド実装 {#client-side}
   ...
   + Adobe Experience Platform Web SDKの導入 {#aep}
      ...
      + [Adobe TargetとWeb SDKを使用したパーソナライゼーション](/help/dev/implement/client-side/aep-web-sdk/target-overview.md)
      + [シングルページアプリケーションの導入](/help/dev/implement/client-side/aep-web-sdk/spa-implementation.md)
      + [アクセス応答トークン](/help/dev/implement/client-side/aep-web-sdk/accessing-response-tokens.md)
      + [mbox3rdPartyIdの使用](/help/dev/implement/client-side/aep-web-sdk/using-mbox-3rdpartyid.md)
      + [at.js ライブラリとPlatform Web SDKの比較](/help/dev/implement/client-side/aep-web-sdk/web-sdk-atjs-comparison.md)
   + at.js の実装 {#at-js-implementation}
      + at.js の仕組み {#at-js}
         + [at.js JavaScript libraryの概要](/help/dev/implement/client-side/atjs/how-atjs-works/overview.md)
         + [at.js worksの概要](/help/dev/implement/client-side/atjs/how-atjs-works/how-atjs-works.md)
...
+ サーバーサイド実装 {#server-side}
   ...
   + [Target SDKの概要](implement/server-side/sdk-guides/getting-started/getting-started.md)
   ...
   + 基本原則 {#core-principles}
      ...
      + [ユーザーIDとグループ化](implement/server-side/sdk-guides/core-principles/user-identification-and-bucketing.md)
      + [オーディエンスターゲティング](implement/server-side/sdk-guides/core-principles/audience-targeting.md)
      + [イベントトラッキング](implement/server-side/sdk-guides/core-principles/event-tracking.md)
      + [ユーザーの権限とプロパティ](implement/server-side/sdk-guides/core-principles/user-permissions-and-properties.md)
   + 統合 {#integration}
   ...
   + オンデバイス判定 {#on-device-decisioning}
      ...
      + ルールアーティファクト {#rule-artifact}
         ...
         + [Adobe Target SDKからダウンロード](implement/server-side/sdk-guides/on-device-decisioning/rule-artifact-sdk.md)
         + [JSON ペイロード経由でダウンロード](implement/server-side/sdk-guides/on-device-decisioning/rule-artifact-json.md)
         + [ルールアーティファクトの例](implement/server-side/sdk-guides/on-device-decisioning/rule-artifact-example.md)
      + [機能フラグによるA/B テストの実施](implement/server-side/sdk-guides/on-device-decisioning/execute-ab-tests-with-feature-flags.md)
      ...
      + [パーソナライゼーションを実現](implement/server-side/sdk-guides/on-device-decisioning/deliver-personalization.md)
   ...
   + Node.js SDK リファレンス {#node-js}
      ...
      + [Node.js SDKの初期化](implement/server-side/node-js/initialize-sdk.md)
      + [オファーの取得（Node.js）](implement/server-side/node-js/get-offers.md)
      + [属性の取得（Node.js）](implement/server-side/node-js/get-attributes.md)
   ...
   + Java SDK リファレンス {#java}
      ...
      + [オファーの取得（Java）](implement/server-side/java/get-offers.md)
      + [属性の取得（Java）](implement/server-side/java/get-attributes.md)
   ...
   + .NET SDK リファレンス {#net}
      ...
      + [.NET SDKの初期化](implement/server-side/net/initialize-sdk.md)
      + [オファーを取得（.NET）](implement/server-side/net/get-offers.md)
      + [属性（.NET）を取得](implement/server-side/net/get-attributes.md)
      + [送信通知（.NET）](implement/server-side/net/send-notifications.md)
   ...
   + Python SDK リファレンス {#python}
      ...
      + [非同期リクエスト （Python）](implement/server-side/python/asynchronous-requests.md)
...
+ Analytics for Target （A4T）とExperience Platform SDK {#a4t}
   + [Adobe Analytics for Target （A4T）でのExperience Platform Web SDKへのログイン](/help/dev/implement/a4t/overview-a4t.md)
   + [Experience Platform Web SDKでのA4T データのクライアントサイドログ](/help/dev/implement/a4t/client-side-logging.md)
   + [Experience Platform Web SDKでのA4T データのサーバーサイドロギング](/help/dev/implement/a4t/server-side-a4t.md)
+ [Recommendations implementation](implement/recommendations/recommendations.md)
+ {hide-from-toc}[Recommendations実装ベータ版](/help/dev/implement/recommendations/recommendations-beta.md)
+ モバイルアプリの導入 {#mobile-apps}
   ...
   + [Web ビューを備えたネイティブアプリで、AEP Mobile SDKを使用してTargetを実装する](/help/dev/implement/mobile/native-app.md)
...
+ API ガイド {#api}
   + [Target APIの概要](/help/dev/before-administer/target-api-overview.md)
   + [Target APIの認証の設定](/help/dev/before-administer/configure-authentication.md)
   + 配信API ガイド {#delivery-api}
      + [配信APIの概要](/help/dev/implement/delivery-api/overview.md)
      + [配信APIと対話するSDK](/help/dev/before-implement/delivery-api-overview/sdks.md)
      + [導入](/help/dev/before-implement/delivery-api-overview/getting-started.md)
      + [ユーザー権限（プレミアム）](/help/dev/before-implement/delivery-api-overview/user-permissions.md)
   ...
   + Admin API {#admin-api}
      + [Admin APIの概要](before-administer/admin-api-overview/admin-api-overview.md)
      + [Adobe Target Admin API](/help/dev/administer/admin-api/admin-api-overview-new.md)
   + プロファイル API {#profile-apis}
      + [Profiles APIの概要](/help/dev/administer/profile-api/profiles-api.md)
      ...
      + [単一プロファイル更新API](/help/dev/administer/profile-api/profile-single-api.md)
      + [プロファイルの一括更新API](/help/dev/administer/profile-api/profile-bulk-api.md)
   + [レポート API](/help/dev/administer/reporting-api/reporting-api.md)
   + Recommendations API {#recommendations-api}
      + [Recommendations APIの概要](before-administer/recs-api/overview.md)
      + [APIによるカタログの管理](before-administer/recs-api/manage-catalog.md)
      + [カスタム条件の管理](before-administer/recs-api/manage-custom-criteria.md)
      + [Recommendationsでの配信APIの使用](before-administer/recs-api/fetch-recs-server-side-delivery-api.md)
      + [Recommendations API](/help/dev/administer/recommendations-api/recommendations-api.md)
   + モデル API {#models-api}
      + [モデル API （ブロックリストへの登録）の概要](before-administer/models-api.md)
...
+ 実装パターン {#implementation-patterns}
   ...
   + at.jsを使用したレコメンデーションの実装パターン {#atjs}
      + [at.jsの概要を使用したレコメンデーションの実装パターン](/help/dev/patterns/recs-atjs/recs-implementation-pattern-atjs.md)
