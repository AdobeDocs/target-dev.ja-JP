...
| [[!UICONTROL targetGlobalSettings(options)]](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md) | `[!UICONTROL targetGlobalSettings()]` UI や REST API を使用して設定する代わりに、[!DNL Target Standard/Premium] を使用して at.js ライブラリで設定を上書きできます。<ul><li>データプロバイダー：この設定では、Demandbase、BlueKai、カスタムサービスなどのサードパーティのデータプロバイダーからデータを収集し、そのデータをグローバル mbox リクエストで mbox パラメーターとして Target に渡すことができます。</li></ul> |
...
| [[!UICONTROL adobe.target.sendNotifications(options)]](/help/dev/implement/client-side/atjs/atjs-functions/adobe-target-sendnotifications-atjs-21.md)<P>（at.js 2.1.0） | この関数は、[!DNL Target] または `[!UICONTROL adobe.target.applyOffer()]` を使用せずにエクスペリエンス `[!UICONTROL adobe.target.applyOffers()]` レンダリングされたときにエッジに通知を送信します。<P>**注意**：この関数は at.js 2.1.0 で導入され、2.1.0 より上のバージョンで使用できるようになります。 |
