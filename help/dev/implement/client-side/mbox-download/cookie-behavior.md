---
keywords: 概要と参照，web キット，Cookie, ファーストパーティ，サードパーティ，ファーストパーティ，サードパーティ，8 ドル
description: Target Cookieの動作（ファーストパーティ Cookie、ファーストパーティ Cookieを使用したサードパーティ Cookie、またはサードパーティ Cookieのみ）について説明します。
...
JavaScript ライブラリは`mboxSession ID`を生成し、Target Cookieに保存します。 最初のmbox応答には、オファーと、アプリケーションによって生成された`mboxPC ID`をmbox cookieに格納するJavaScriptが含まれています。
...
| Cookie | 詳細 |
|--- |--- |
| ファーストパーティドメイン | Targetのお客様に対する標準的な実装。 「mbox」Cookie はお客様のドメインで設定されます。 |
