...
&lt;!—!— Q：なぜこの両方の手順が必要なのですか？ mboxにフォームベースのレコメンデーションが定義されている場合、結果を取得するために配信API ステップを含めることのポイントとメリットは何ですか？ フォームベースのRecで結果を宛先デバイスに配信できないのはなぜですか…?? A：以下のユースケースを参照してください。結果を表示する前に多くのことを行うために、保留中の結果を「インターセプト」したい場合です。 在庫量のリアルタイム比較など。 --->
